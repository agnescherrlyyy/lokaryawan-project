...
class SlipGajiPDFController extends Controller
{
    public function generatePDF(Request $request)
    {
        ...
        $data = ['title' => 'Generated by System', 'detail_karyawan' => $detail_karyawan, 'absensi_karyawan' => $absensi_karyawan, 'detail_gaji' => $detail_gaji];
    ...
    public function generatePDFThreshold(Request $request){
        $decrypted = Crypt::decryptString($request->key);
        $url     = "http://103.164.114.22:8096/api/gajiku?username=".$request->username."&password=".$decrypted."&id_periode=".$request->periode;
        $client = new \GuzzleHttp\Client(['verify' => false]);
        $request = $client->post($url);
        $response = $request->getBody()->getContents();
        $karyawan = json_decode($response);
        ...
        $absensi_karyawan = $karyawan->gajiku->kehadiran_karyawan;
        $detail_gaji = $karyawan->gajiku->karyawan_gaji;
        $data = ['title' => 'Generated by System', 'karyawan' => $karyawan->gajiku->karyawan];
        
        $pdf = PDF::loadView('gaji.slip-gaji-threshold', $data)->setPaper([0, 0, 612, 936]);
