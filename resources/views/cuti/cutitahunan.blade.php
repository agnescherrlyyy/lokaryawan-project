...
@section('content')
    <!-- Heading Start -->
    <div
        class="max-w-full w-full px-5 lg:pl-10 lg:pr-16 lg:flex lg:items-start lg:justify-between py-4 bg-white dark:bg-slate-800">
        <div class="w-full">
    ...
    <section class="block mt-9 px-2 lg:pl-10 lg:pr-10">
        <div class="w-full mb-4 px-3 flex flex-col md:flex-row md:items-center gap-4">
            <button id="riwayat-cuti"
                class="w-full md:w-fit px-4 py-3 rounded-full bg-sekunder-60 text-slate-50 font-medium text-sm hover:bg-sekunder-40 flex items-center justify-center gap-2">
            ...
            <button id="permintaan-cuti"
                class="w-full md:w-fit px-4 py-3 rounded-full bg-primer-60 text-slate-50 font-medium text-sm hover:bg-primer-40 flex items-center justify-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                    <path fill-rule="evenodd"
        ...
        <div class="w-full flex flex-col items-center justify-center rounded-lg">
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="w-full flex flex-col gap-4 px-6 py-6 bg-white dark:bg-slate-800 rounded-lg">
                    ...
                    <div
                        class="w-full flex flex-col md:flex-row md:gap-5 md:items-center gap-3 pt-4 border-t border-t-slate-300 dark:border-t-slate-600">
                        <div class="w-10 h-10 md:w-12 md:h-12">
                            <img src="{{ asset('img/izin.svg') }}" alt="icon" class="w-full h-full">
                        </div>
                        <div class="flex flex-col gap-1">
        ...
        <div class="w-full flex flex-col xl:flex-row gap-4 mt-6">
            ...
            <div id="selected-date"
                class="w-full xl:w-3/5 p-4 lg:py-4 lg:px-6 bg-white dark:bg-slate-800 rounded-lg max-h-fit h-full">
                ...
                <div class="w-full flex flex-col gap-2">
                    ...
                    <form action="" method="POST">
                        <textarea name="alasan-cuti" id="alasan-cuti" cols="10" rows="3" class="hidden"
                            placeholder="Masukan Alasan Mengambil Cuti"></textarea>
                    </form>
                </div>
                <div class="w-full pt-2">
                    <button id="btn-cuti" type="submit"
                        class="text-center px-4 py-3 w-full rounded-md font-semibold text-sm text-white mt-2 bg-primer-60 hover:bg-primer-80 transition-all duration-200 ease-in-out uppercase">AJUKAN
                        CUTI</button>
                </div>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-cuti').click(function(e) {
                ...
                const decryptedBytes = CryptoJS.AES.decrypt(encryptedSelectedDates,
                    '{{ env('APP_KEY') }}');
                ...
                if (!fromData.id_karyawan || !fromData.id_cuti || !fromData.tipe_cuti || !fromData.cuti || !
                    alasanCuti) {
                    Swal.fire({
                        title: 'Oops!',
                        text: 'Harap Lengkapi Data Pengajuan Cuti Terlebih Dahulu',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.data === "ID Karyawan Tidak Ditemukan") {
                            ...
                            $('#expired-past').text(cutiTahunan.date_start + '  s/d  ' + cutiTahunan
                                .date_expired);
                            ...
                            const encryptedSisaCuti = CryptoJS.AES.encrypt(JSON.stringify(sisaCuti),
                            ...
                            $('#expired-past').text(cutiTahunanPertama.date_start + '  s/d  ' +
                                cutiTahunanPertama.date_expired);
                            ...
                            $('#expired-now').text(cutiTahunanKedua.date_start + '  s/d  ' +
                                cutiTahunanKedua.date_expired);
        ...
        function getSisaCuti(sisaCutiDisimpan) {
            const encryptedSisaCuti = CryptoJS.AES.encrypt(JSON.stringify(sisaCutiDisimpan), '{{ env('APP_KEY') }}')
                .toString();
            localStorage.setItem('encryptedSisaCuti', encryptedSisaCuti);
        }
    </script>
