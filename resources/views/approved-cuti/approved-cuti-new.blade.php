    ...
    <section class="block mt-9 px-5 lg:pl-10 lg:pr-16">
        ...
        <div class="w-full">
            <div class="w-full bg-white dark:bg-slate-800 rounded-lg mt-9">
                <div
                    class="w-full flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 px-6 py-6 border-b border-slate-200 dark:border-slate-700">
                ...
                <div class="w-full px-6">
                    <div class="w-full flex flex-col gap-4">
                        ...
                        <div class="w-full flex flex-wrap items-center gap-2">
                            ...
                            <span id="cuti"
                                class="block w-fit px-3 py-2 bg-blue-50 text-blue-600 rounded-full font-semibold text-xs">

                            </span>
                        </div>
                        <div class="w-full flex items-center gap-2">
                            <svg class="h-5 w-5 text-slate-600 dark:text-slate-300" width="24" height="24"
                ...
                <div class="w-full lg:w-1/2 lg:flex-row flex flex-col gap-4 px-6 mt-6 pb-6">
                    <button id="reject" type="submit"
                        class="btn-modal text-center px-4 py-3 w-full rounded-full font-semibold text-sm text-white mt-2 bg-rose-600 hover:bg-rose-800 transition-all duration-200 ease-in-out uppercase">rejected</button>
                    <button id="approve" type="submit" value="1"
                        class="text-center px-4 py-3 w-full rounded-full font-semibold text-sm text-white mt-2 bg-primer-60 hover:bg-primer-80 transition-all duration-200 ease-in-out uppercase">Approved</button>
                </div>
            </div>
        </div>
    </section>
...
@section('modal')
    <section id="modalAdd"
        class="modal-container hidden max-w-full w-full max-h-screen h-full fixed top-0 left-0 z-[100]">
        <div class="w-full h-full flex items-center justify-center px-5">
            <div id="reject"
                class="show-modal w-full max-w-full md:max-w-lg rounded-lg bg-white dark:bg-slate-700 p-4">
                ...
                <div class="w-full pt-6">
                    <form id="form_add" class="w-full flex flex-col items-center justify-center gap-4">
                        ...
                        <div class="w-full flex flex-col justify-center items-start gap-2 px-4">
                            ...
                            <div class="w-full">
                                <textarea name="alasan" id="alasan" class="item-input" cols="30" rows="10"
                                    placeholder="Masukan Alasan Menolak Cuti"></textarea>
                            </div>
                        </div>
                    </form>
                </div>
                <div class="w-full flex item-center justify-end gap-3 mt-6 pb-4">
                    ...
                    <button value="2" id="submit-reject" type="submit" form="form_add"
...
@section('script')
    ...
    <script type="text/javascript">
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    if (statusAction === "0") {
                        ...
                        function actionCuti(idCutiTrn, statusApproved, noteApprove, username, textError,
                            textSuccess) {
                            $.ajax({
                        ...
                        $("#submit-reject").click(function(e) {
                            ...
                            actionCuti(idCutiTrn, statusReject, noteReject, username, textError,
                                textSuccess);
                        ...
                        $('#approve').click(function(e) {
                            ...
                            var formattedDate = day + "-" + (month < 10 ? "0" : "") + month + "-" +
                                year;
                            var successMessage =
                                `Successfully approved leave request on ${formattedDate}.`;

                            var textError = 'Menyetujui Cuti Karyawan Gagal, Silahkan Coba Lagi';
                            var textSuccess = 'Menyetujui Cuti Karyawan Berhasil';
                            console.log(idCutiTrn, statusApproved, noteApprove, username, textError,
                                textSuccess);
                            ...
                            actionCuti(idCutiTrn, statusApproved, noteApprove, username, textError,
                                textSuccess);
                            $("#reject").prop("disabled", true);
                            $("#approve").prop("disabled", true);
                        });
        ...
        function getTanggalRange(tanggalCuti) {
            ...
            const formattedDates = sortedDates.map(date => formatTanggal(date));
