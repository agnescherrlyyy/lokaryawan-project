...
<body class="font-Poppins">
    <section id="bg-auth"
        class="max-w-full w-full min-h-screen h-full flex flex-col items-center justify-center px-5 lg:px-16">
        ...
        <div class="w-full flex items-center justify-center z-50">
            <div class="max-w-sm w-full bg-white px-2 lg:px-6 lg:py-6 rounded-lg overflow-hidden lg:shadow-lg">
                ...
                <p class="text-sm text-slate-400 mt-2">Sugeng Rawuh, silahkan berikan informasi untuk akses aplikasi 👋
                </p>
                <div class="w-full mt-4">
                    <form action="" method="POST">
                        <div class="w-full mb-4 relative">
                            <label htmlFor="username"
                                class="block mb-2 font-medium text-slate-700 text-sm">Username</label>
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                class="w-5 h-5 absolute left-3 top-10 text-slate-700">
                        ...
                        <div class="w-full mb-4 relative">
                            <label htmlFor="password"
                                class="block mb-2 font-medium text-slate-700 text-sm">Password</label>
                        ...
                        <button id="btn-login" type="submit"
                            class="text-center px-4 py-3 w-full rounded-md font-semibold text-sm text-white mt-6 bg-primer-60 hover:bg-primer-80 transition-all duration-200 ease-in-out">Login</button>
                    </form>
                </div>
            </div>
        </div>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.status == "success") {
                            const encryptedFromData = CryptoJS.AES.encrypt(JSON.stringify(
                                formData), '{{ env('APP_KEY') }}').toString();
                            ...
                            window.location.href = "{{ url('start-session') }}?name=" + data
                                .user.name + "&username=" + data.user.username;
                        } else {
                            Swal.fire({
                                title: 'Login Gagal',
                                text: data.message,
